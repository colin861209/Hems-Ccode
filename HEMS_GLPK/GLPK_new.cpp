...
/*		float solar2[96] = {
	...
	if (sample_time == 0)
	{
		for (i = 0; i < time_block; i++)
		{
			...
			//snprintf(sql_buffer, sizeof(sql_buffer), "INSERT INTO solar_day (time_block,value) VALUES(%d,%.3f)", i, solar2[i]);
	...
	*/
	...
	//    //{SOC_ini = atof(mysql_row[0])/100.0;}					//Àò¨ú¦¹¨è¥i¨Ï¥Î¤§¹q¦ÀSOC
	//	//SOC_ini = 0.78;
	...
	//	//SOC_ini = atof(mysql_row[0]) / 100.0;
	//	printf("SOC:%f\n", SOC_ini);
	//	//   SOC_ini = 0.85;
	//	memset(sql_buffer, 0, sizeof(sql_buffer));
	//	mysql_free_result(mysql_result);
	...
	if (sample_time != 0)
	{
		for (i = 0; i < uninterrupt_num; i++)
		{
			...
			/*snprintf(sql_buffer, sizeof(sql_buffer), "SELECT %s FROM control_status WHERE equip_id = '%d'", column, (i + app_count + 12 + (ponit_num-1) * 2 ));*/ //MUSTMUSTMUST CHANGE(if add rows)
		...
		for (i = 0; i < varying_num; i++)
		{
			...
			/*snprintf(sql_buffer, sizeof(sql_buffer), "SELECT %s FROM control_status WHERE equip_id = '%d'", column, (i + app_count + 12 + (ponit_num - 1) * 2 + uninterrupt_num));*/ //MUSTMUSTMUST CHANGE(if add rows)
	...
	//½æ¹q­­¨î¦¡ ¥u¯à½æ¨Ó¦Û¤Ó¶§¯à»P¿U®Æ¹q¦À¤§¹q¤O(Sell to grid constraint,Psell<=Psolar+Pfc)
	for (i = 0; i < (time_block - sample_time); i++)
	{
		...
		if (uninterrupt_flag[h] == 1)
		{
			if ((uninterrupt_end[h] - sample_time) >= 0)
			{
				if ((uninterrupt_start[h] - sample_time) <= 0)
				{
					for (i = 0; i <= (uninterrupt_end[h] - sample_time); i++)
					{
						power1[(time_block - sample_time) * (15 + n) + app_count + 2 + counter + i][i * variable + h + interrupt_num] = 1.0;										//¤£¥i¤¤Â_­t¸ü¨Mµ¦ÅÜ¼Æ
					}
				}
				n += 1;
			}
		...
		if (varying_flag[h] == 1)
		{
			if ((varying_end[h] - sample_time) >= 0)
			{
				if ((varying_start[h] - sample_time) <= 0)
				{
					for (i = 0; i <= (varying_end[h] - sample_time); i++)
					{
						power1[(time_block - sample_time) * (15 + n) + app_count + 2 + counter + i][i*variable + h + (interrupt_num + uninterrupt_num)] = 1.0;					//ÅÜ°Ê­t¸ü¨Mµ¦ÅÜ¼Æ
					}
				}
				n += 1;
			}
			...
			if ((varying_end[h] - sample_time) >= 0)
			{
				if ((varying_start[h] - sample_time) >= 0)
				{
					for (i = (varying_start[h] - sample_time); i <= (varying_end[h] - sample_time); i++)
					{
						...
						power1[(time_block - sample_time) * (15 + n) + app_count + 2 + counter + i][(i * variable) + h + (variable - varying_num)] = 1.0;    //»²§UÅÜ¼ÆPa 
					}
				}
				else if ((varying_start[h] - sample_time) < 0)
				{
	...
	for (i = 0; i < (time_block - sample_time); i++)
	{
		...
		glp_set_col_bnds(mip, ((app_count + 6) + i*variable), GLP_DB, -0.0000000001, Psell_max);							                //ÃB¥~¹q¤O//if no sell
		...
		{
			glp_set_col_bnds(mip, ((app_count + 14 + piecewise_num * 2 + j) + i*variable), GLP_DB, 0.0, 1.0);	                 //¤£¥i¤¤Â_­t¸ü»²§U¤G¤¸ÅÜ¼Æ
			glp_set_col_kind(mip, ((app_count + 14 + piecewise_num * 2 + j) + i*variable), GLP_BV);
		}

		for (j = 1; j <= varying_num; j++)
		{
			glp_set_col_bnds(mip, ((app_count + 14 + piecewise_num * 2 + uninterrupt_num + j) + i*variable), GLP_DB, 0.0, 1.0);						//ÅÜ°Ê­t¸ü»²§U¤G¤¸ÅÜ¼Æ
		...
		for (j = 1; j <= varying_num; j++)
		{
			...
			glp_set_col_bnds(mip, ((app_count + 14 + piecewise_num * 2 + uninterrupt_num + varying_num + j) + i*variable), GLP_DB, 0.0, varying_p_max[j - 1]);				//ÅÜ°Ê­t¸ü¯Ó¯àÅÜ¼ÆPa	
